    ...
    while not done:
        ...
        for op in OpCode.get_all_ops():
            ...
            if test_bank == after_value:
                candidates.add(op)
        instruction_no = instruction[0]
        operator_candidates.setdefault(instruction_no, candidates)
        operator_candidates[instruction_no] = operator_candidates[instruction_no] & candidates
    handled = set()
    while any(
        len(candidate_set) != 1 
        for candidate_set in operator_candidates.values()
    ):
        has_single = set(
            candidate
            for candidate in operator_candidates
            if len(operator_candidates[candidate]) == 1
        )
        has_single -= handled
        handling = has_single.pop()
        
        for candidate_set in operator_candidates.values():
            if candidate_set == operator_candidates[handling]:
                continue
            candidate_set -= operator_candidates[handling]
        handled.add(handling)

    operator_value_map = {
        operator_no: operator_candidates[operator_no].pop()
        for operator_no in operator_candidates
    }
    return operator_value_map

operator_value_map = run_tests(puzzle_input)
print(operator_value_map)
register_bank = [0]*4
for instruction_line in puzzle_input:
    if not instruction_line:
        continue
    instruction = list(map(int, instruction_line.split(" ")))
    operator = operator_value_map[instruction[0]]
    operator = operator_value_map[instruction[0]]
    OpCode.perform_op(operator, *instruction[1:], registers=register_bank)

# Result
print(register_bank[0])