...
class NestedBags:
    ...
    def contains(self, color, top=True):
        
        return any(bag.color == color or bag.contains(color, top=False) for bag in self.inner_bags.keys())
...
# Actual Code
bags = set(map(NestedBags.parse_rule, puzzle_input))

# Result
print(sum(map(methodcaller("contains", "shiny gold"), bags)))
