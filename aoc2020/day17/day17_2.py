    ...
    def from_map(dim_map):
        ...
        for ridx, row in enumerate(dim_map):
            for cidx, cube_state in enumerate(row):
                ...
                cube.set_neighbours(tuple(dimension.cubes[tuple(map(sum, zip(pos, direction)))] for direction in all_directions(4) if direction != (0,0,0,0)))
        return dimension
    ...
    def tick(self):
        ...
        for pos, cube in cubes:
            if not cube.neighbours:
                cube.set_neighbours(tuple(self.cubes[tuple(map(sum, zip(pos, direction)))] for direction in all_directions(4) if direction != (0,0,0,0)))
            cube.get_next_state()
        for _, cube in cubes:
            cube.tick()
