...
def best_min(guard):
  ...
  return max(range(60), key=lambda t: minutes_asleep[t])

def total_sleep_time(guard):
  total_sleep = sum(map(lambda g: g["sleep_time"], guard_stats[guard]), timedelta(0))
  return total_sleep
guards.sort(key=total_sleep_time)
# Result Parsing
guard = guards[-1]
most_sleep_min = best_min(guards[-1])
